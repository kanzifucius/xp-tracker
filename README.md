...
For production, you should scope the ClusterRole down to only the specific API groups and resources you need:
...
1. **Test & Lint** -- `go vet`, `golangci-lint`, `go test -race`
...
2. **Build, Push & Release** -- Builds multi-arch image, pushes to GHCR, creates a GitHub Release
